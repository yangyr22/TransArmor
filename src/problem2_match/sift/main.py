...
#         gaussian_image = gaussian_images[keypoint.octave][keypoint.image]
#         num_rows, num_cols = gaussian_image.shape
#         x = int(round(keypoint.scale *keypoint.pt[0]))
#         y = int(round(keypoint.scale *keypoint.pt[1]))
#         point = np.array([x,y])
...
#         hist_width = scale_multiplier * 0.5 * keypoint.scale * keypoint.size
...
#                     window_row = int(round(point[0] + row))
#                     window_col = int(round(point[1] + col))
...
class SIFT(object):
    ...
    def vis(self, img):
        ...
        new_img = np.copy(img)
        for keypoint in keypoints:
            draw_a_keypoint(new_img, keypoint)
        cv2.cvtColor(new_img, cv2.COLOR_RGB2BGR, new_img)
        return new_img
